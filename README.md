...
2. Second project: UDP Pinger Client Impelementations
	...
	Based on Python3 based UDP ping client, as seen in the image above, the server is running on port 12000. The client is running on localhost, while the server is running on the same machine. Both sides (server and client) are showing in what UNIX time both are communicating, as well as the sequence number.
